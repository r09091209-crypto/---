...
const initialUsers = [
  ...
  {id:"u3", nickname:"cipher_x", email:"cipher@void.org", role:"admin", status:"active"},
...
function load(){ try{ const r=localStorage.getItem(KEY); if(r) Object.assign(state, JSON.parse(r)); }catch(e){} }
...
function login(email, nickname){
  ...
  if(!existing){ existing={id:"u"+Date.now(), nickname:nickname||email.split("@")[0], email, role:"user", status:"active"}; state.users.push(existing); }
...
function renderAdmin(){
  if(!isAdmin()){ app.innerHTML=`<section class="container section"><div class="card">⛔ Доступ только для администраторов. Войди как cipher@void.org</div></section>`; return; }
  app.innerHTML = `
    <section class="container section">
...
document.getElementById("authForm").addEventListener("submit", e=>{
  e.preventDefault();
  login(document.getElementById("email").value.trim(), document.getElementById("nickname").value.trim());
});
